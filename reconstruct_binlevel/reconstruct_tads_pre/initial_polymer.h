...
//     // string refer_file="/data/home/txguo/data_use/data_final/normalize/KRNorm/chr1_krnorm.mtx";
...
//     cout<<"length:"<<reference_contact_s.cols()<<'\n';
// }
...
void initial_refer(string chr){
    ...
    // string  refer_file="/data/home/txguo/data_use/maxEnt/reference/reference_use/chr"+chr+"_20k.mtx";
    loadMarket(reference_contact_s,refer_file);
    // reference_contact_s=reference_contact_s.selfadjointView<Eigen::Upper>();
    reference_contact_s=reference_contact_s.triangularView<Eigen::StrictlyUpper>();
...
void initial_neighbor(){


    //initial tad_neighbor_contact
    vector<unordered_map<int, vector<int>>>(number_of_threads).swap(tad_neighbor_contact); 
    vector<unordered_map<pair<int, int>, float, pair_hash>>(number_of_threads).swap(neighbor_contact_dis);
    
    // int x;int y;int z;
    // vector<int> tad_neighbor_contact_initial;   

    // for(int l=0;l<number_of_threads;l++){
    //     //update position2site_index
    //     position2site_index[l].resize(pol_length);
    //     fill(position2site_index[l].begin(), position2site_index[l].end(), 0);    
    //     for (int i=0;i<pol_length;i++){
    //         if (site_available[l][i]==1){
    //             position2site_index[l][site_index2position[l][i]]=i;
    //         }
    //     }                 

    //     //initial neighbor contact
    //     neighbor_contact_dis_mat[l]=Eigen::MatrixXf::Zero(tad_bin_num,tad_bin_num);
    //     neighbor_contact_m_mat[l]=Eigen::MatrixXi::Zero(tad_bin_num,tad_bin_num);

    //     // for (int i=0;i<polymer[l].size();i++){
    //     //     if (position2site_index[l][i]%6==0){
    //     //         x=polymer[l][i](0);y=polymer[l][i](1);z=polymer[l][i](2);
    //     //         find_neighbor(x,y,z,position2site_index[l][i],0,polymer[l],site_index2position[l],tad_neighbor_contact_initial,neighbor_contact_dis[l]);
    //     //         //update
    //     //         if (tad_neighbor_contact_initial.empty()==0){
    //     //             tad_neighbor_contact[l][position2site_index[l][i]]=tad_neighbor_contact_initial;
    //     //         }    

    //     //         for(auto elem:neighbor_contact_dis[l]){
    //     //             int sitei=elem.first.first;
    //     //             int sitej=elem.first.second;
    //     //             float dis=elem.second;
                    
    //     //             neighbor_contact_dis_mat[l](sitei/6,sitej/6)=dis;
    //     //             neighbor_contact_dis_mat[l](sitej/6,sitei/6)=dis;
    //     //         }
