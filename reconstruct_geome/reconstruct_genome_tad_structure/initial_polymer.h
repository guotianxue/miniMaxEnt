...
void initial_structureByfile(int thread_num,vector<Eigen::MatrixXi> &connected_structure_chr){
    string file="/data/home/txguo/data_use/maxEnt/whole_chr/chr_tads_structure/";
    string structure_file;
    int x,y,z;
    for(int chr_homology=0;chr_homology<chr_homology_num;chr_homology++){
        connected_structure_chr[chr_homology]=MatrixXi::Zero(chr_len[chr_homology/2],3);

        int i=0;       
        int k;   
        structure_file=file+to_string(chr_homology+1)+"/polymer"+to_string(thread_num)+".txt";   
        // cout<<thread_num<<'\t'<<chr_homology<<'\t'<<chr_homology_num<<'\t'<<structure_file<<'\n'; 
        ifstream initial_structure;     
        initial_structure.open(structure_file);
        while(initial_structure>> k >> x >> y >> z ){
            connected_structure_chr[chr_homology](i,0)=x;
            connected_structure_chr[chr_homology](i,1)=y;
            connected_structure_chr[chr_homology](i,2)=z;
            i+=1;
        }
...
void initial_tads_center(int thread_num){
    ...
    for(int chr_index=0;chr_index<chr_num;chr_index++){
        int length=chr_len[chr_index];    

        for(int homology=0;homology<2;homology++){
            homology_chr_index=chr_index*2+homology;   
            x_c=connected_structure_chr[thread_num][homology_chr_index].col(0).sum()/(length);
            y_c=connected_structure_chr[thread_num][homology_chr_index].col(1).sum()/(length);
            z_c=connected_structure_chr[thread_num][homology_chr_index].col(2).sum()/(length);
    ...
    // cout<<tads_center_chr[0][43]<<'\n';

    //initial tads total contacts
    tads_total_contacts_s[thread_num].resize(tads_bound_genome.size(),tads_bound_genome.size());
    tads_total_contacts_s[thread_num].setZero();
       
}

void initial_E_By_structure(){

    //setzero contact matrix
    vector<vector<float>> contact_matrix(tads_num,vector<float>(tads_num,0));
    haploid_contact_matrix.swap(contact_matrix); 
    float dis;     

    //dis 2 contact matrix
    for(int l=0;l<number_of_threads;l++){
        for(int i=0;i<tads_num;i++){
            for(int j=i+1;j<tads_num;j++){
                dis=homology_dis_matrix[l][i][j];
                haploid_contact_matrix[i][j]+=1/dis/dis;
            }          
        }        
    }
    normalize_tads();
...
void initial_tads_byfile(int thread_num,vector<vector<Eigen::MatrixXi>> &tads_chr,vector<Eigen::MatrixXi> &connected_structure_chr,string files){
    ...
    //initial connected_structure_chr by file
    if(structure_initial_mark==1){
        initial_structureByfile(thread_num,connected_structure_chr);
    }

    //initial homology_m
    vector<int> h_m(chr_homology_num,-1);
    homology_m[thread_num].swap(h_m);

    //initial homology_contact_matrix
    vector<vector<float>> contact(tads_num,vector<float>(tads_num,0));
    homology_contact_matrix[thread_num]=contact;
