...
float get_disAenergy(int thread_num,int chr_homology,int tad_index_start,int tad_index,int chr_neighbor_homology,int dir,vector<float> &new_center_vec,vector<MatrixXf> &tads_center_chr,vector<vector<float>> &homology_dis_matrix){
    ...
    for(int tad_neighbor_index=0;tad_neighbor_index<tad_neighbor_chr_len;tad_neighbor_index++){
        ...
        homology_dis_matrix_change[thread_num][indexi][indexj]=dis;             
    ...
    if(tad_index>=tad_len/2){
        ...
        for(int i=tad_index;i<tad_len;i++){    
            for(int chr_neighbor=0;chr_neighbor<chr_homology_num;chr_neighbor++){ 
                float pre=energy_change;

                energy_change+=get_disAenergy(thread_num,chr_homology,tad_index,i,chr_neighbor,1,new_center_vec,tads_center_chr,homology_dis_matrix); 
        ...
        for(int i=0;i<tad_index+1;i++){    
            for(int chr_neighbor=0;chr_neighbor<chr_homology_num;chr_neighbor++){ 
                ...
                energy_change+=get_disAenergy(thread_num,chr_homology,tad_index,i,chr_neighbor,0,new_center_vec,tads_center_chr,homology_dis_matrix);  
...
void update_neighbor_contact(int homology,int thread_num,int m){
    ...
    for(int i=homology_start;i<homology_end;i++){
        for(int j=0;j<tads_num;j++){
            ...
            contact=float(m-m_pre)/pow(dis,2);
...
void normalize_tads_reference() {
    ...
    int length=tads_bound_genome.size();
    float sum=reference_genome_tads_contact_s.sum();
    reference_genome_tads_contact_s*=float(length)/(2*sum);
    cout<<"refer_check"<<"\t"<<reference_genome_tads_contact_s.sum()<<"\n";

    MatrixXf reference_genome_tads_contact_dense=MatrixXf(reference_genome_tads_contact_s);
...
void update_tads_energies() {
    ...
    for (int i = 0; i < length; i++) {
        ...
        for (int j = i+1; j < length; j++) {
            ...
            chr_E[i][j] += learning_rate * sqrt(1/(max(reference_genome_tads_contact[i][j],float(0.000001))))*margin;
            chr_E[j][i] = chr_E[i][j];
            loss+=fabs(margin);        
    ...
    //     int chr_start=haploid_tad_start[chr];
    //     int chr_end=chr_tad_len[chr]+chr_start;

    //     for(int i=chr_start;i<chr_end;i++){
    //         for(int j=i+1;j<chr_end;j++){
    //         margin=haploid_contact_matrix[i][j]-reference_genome_tads_contact[i][j];         
    ...
    // float shift=(2*(Interaction_E_tads.cwiseProduct(reference_tads_contact_s)) / length).sum();

    // cout << "Shift: " << shift << endl;
    // //update E
    // Interaction_E_tads-=shift*One;
    // cout<<"lala:"<<(Interaction_E_tads).cwiseProduct(reference_tads_contact_s+SparseMatrix<float>(reference_tads_contact_s.transpose())).sum()<<'\n';
