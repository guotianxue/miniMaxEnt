...
void chrom_rotate(Eigen::MatrixXf &chrom,int rotate_index){
    ...
    int x,y,z;
    int length=chrom.rows();
    Eigen::MatrixXf center_point(1,3);

    //rotate
    rotate_mat=rotate[rotate_index].replicate(chrom.rows(), 1); 

    x=(chrom.col(0).sum())/(length);
    y=(chrom.col(1).sum())/(length);
    z=(chrom.col(2).sum())/(length);   

    center_point<<float(x),float(y),float(z);
    Eigen::MatrixXf center=center_point.replicate(chrom.rows(), 1);
    chrom=(chrom-center).cwiseProduct(rotate_mat)+center;
...
void chrom_rotate(Eigen::MatrixXi &chrom,int rotate_index){
    ...
    int x,y,z;
    int length=chrom.rows();
    Eigen::MatrixXi center_point(1,3);
    ...
    center_point<<x,y,z;
    Eigen::MatrixXi center=center_point.replicate(chrom.rows(), 1);
    chrom=(chrom-center).cwiseProduct(rotate_mat)+center;
...
void chromosome_move(int thread_num,vector<MatrixXi> &connected_structure_chr,vector<vector<MatrixXi>> &tads_chr,vector<MatrixXf> &tads_center_chr,vector<MatrixXi> &dir_match,int chr_homology,int m){
    ...
    Eigen::MatrixXf new_chromosome_center=tads_center_chr[chr_homology];
    ...
    chrom_move(new_chromosome_center,move_index);
    // chrom_move(new_chromosome,move_index);
    float energy=get_chrom_delta_E(thread_num,chr_homology,new_chromosome_center,tads_center_chr,homology_dis_matrix[thread_num]);
    float sphere_limition=sphere_limit(new_chromosome_center,alpha)-sphere_limit(tads_center_chr[chr_homology],alpha);
                                                                                                                                   
    if(accept_move(energy+sphere_limition)){

        //update center      
        tads_center_chr[chr_homology]=new_chromosome_center;

        //update bin_location
        chrom_move(connected_structure_chr[chr_homology],move_index);
...
void chromosome_rotate(int thread_num,vector<MatrixXi> &connected_structure_chr,vector<vector<MatrixXi>> &tads_chr,vector<MatrixXf> &tads_center_chr,vector<MatrixXi> &dir_match,int chr_homology,int m){
    ...
    Eigen::MatrixXf new_chromosome_center=tads_center_chr[chr_homology];
    // Eigen::MatrixXi new_chromosome=connected_structure_chr[chr_homology];    
    ...
    chrom_rotate(new_chromosome_center,rotate_index);      
