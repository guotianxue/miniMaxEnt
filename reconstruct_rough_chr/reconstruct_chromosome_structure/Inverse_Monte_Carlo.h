...
void chrom_tads_move(int thread_num,int m){ //performs a single Monte Carlo step
    ...
    // uniform_int_distribution<int> unichrom(0,chr_num-1);
    // int chr_index=unichrom(gen);
    ...
    chrom_rotate_move(thread_num,connected_structure_chr[thread_num],tads_chr[thread_num],tads_center_chr[thread_num],dir_match[thread_num],chr_homology,tad_index,m);    

    // if(move_type==0){
    //     chromosome_move(thread_num,connected_structure_chr[thread_num],tads_chr[thread_num],tads_center_chr[thread_num],dir_match[thread_num],chr_index,m);
    // }
    // else{
...
void tads_move2chrom(int thread_num,int m,int chr_homology){ //performs a single Monte Carlo step

    float frequence=unif(gen);

    int homology_tad_num=chr_tad_len[chr_homology/2];
    uniform_int_distribution<int> unitad(0,homology_tad_num-1);
    int tad_index=unitad(gen);
...
void run_tads2chrom(int thread_num, int mc_moves,int chr_homology) { //burns in the polymer configurations

    for (int m = 1; m < mc_moves; m++) {
        tads_move2chrom(thread_num,m,chr_homology);
    } 
    ...
    update_neighbor_contact(chr_homology,thread_num,mc_moves);          
