...
float learning_rate=0.05;//0.05 0.4
...
void structure_initial(int start_bin,int end_bin,int model){//model=1 initial tads ; model=2 initial all chr
    ...
    cout<<start_bin<<"\t"<<end_bin<<'\t'<<reference_contact_s.cols()<<"\n";
    ...
    for (auto l = 0; l < number_of_threads; l++) {
        threads[l] = thread(initial_neighbor, l);   
    }
    for (auto &&l : threads) {
        l.join();
    }        
...
void reconstruct(){;
    ...
    if (not initial_tads){structure_initial(start_bin,end_bin,2);}
    else{structure_initial(start_bin,end_bin,1);}
    ...
    for (int n =0; n<update_steps;n++) { //do iterative update scheme
        ...
        //弥补偏差
        if(not initial_tads){
            MatrixXf bias(tad_bin_num,tad_bin_num);
            bias.fill(0.00001f);
            tad_final_contacts_s=tad_final_contacts_s+SparseMatrix<float>(bias.cwiseProduct(mask_s));                   
        }
        ...
        string contacts_out="/data/home/txguo/code_final/reconstruct_tads/final.txt";
        ...
        // SparseMatrix<float> A=tad_final_contacts_s+SparseMatrix<float>(reference_one_tad_contact_s.transpose());
        // Eigen::saveMarket(A, "./test.mtx");           
    
        // //output polymer
        // int pol_index=0;
        // for(int l=0;l<number_of_threads;l++){
        //     string polymer_out="./polymer/"+chr +"/chr"+chr+"_"+to_string(l)+".txt" ;
    ...
    if(initial_tads){
        ...
        // //output refer
        // string contacts_refer=outfile+"/chr"+chr + "_refer/chr" + chr + "_" + to_string(start_bin)+ "_"+ to_string(end_bin)+ ".mtx" ;  
        // SparseMatrix<float> contact_r=reference_one_tad_contact_s+SparseMatrix<float>(reference_one_tad_contact_s.transpose());
        // Eigen::saveMarket(contact_r, contacts_refer);            
